        ...
        fs.readdir(path.join(directory, type), function (err, files) {
          ...
          async.eachSeries(files, function (file, fileCallback) {
            ...
            if (type == "msg") {
              ...
              cachehit = getMessageFromRegistry(messageType, [type, "Response"]) != undefined && getMessageFromRegistry(messageType, [type, "Request"]) != undefined;
            }
            if (cachehit) {
              fileCallback();
            } else {
...
function getMessageFromFile(messageType, filePath, type, callback) {
  ...
  parseMessageFile(filePath, details, type, function (error, details) {
    if (error) {
      callback(error);
    } else {
      if (type == "msg") {
        ...
        setMessageInRegistry(messageType, function () {
          return request.md5sum();
        }, type, 'md5sum');
        setMessageInRegistry(messageType, function () {
          return messageType;
        }, type, 'datatype');
...
function buildMessageClass(details) {
        ...
        if (field.messageType) {
          ...
          if (values && typeof values[field.name] != "undefined") {
            ...
            if (match) {
              ...
              that[field.name] = new field.messageType(values[field.name]);
            }
          } else {
            // use defaults
            if (match) {
  ...
  Message.Constants = Message.constants = Message.prototype.constants = function () {
    var ret = {};
  ...
  Message.serialize = Message.prototype.serialize = function (obj, buffer, offset) {
  ...
  Message.getMessageSize = function (msg) {
    return fieldsUtil.getMessageSize(msg);
  };
