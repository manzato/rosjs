...
var TcprosUtils = {
  createSubHeader: function createSubHeader(callerId, md5sum, topic, type) {
    var fields = [callerIdPrefix + callerId, md5Prefix + md5sum, topicPrefix + topic, typePrefix + type];
    return serializeStringFields(fields);
  },
  createPubHeader: function createPubHeader(callerId, md5sum, type, latching) {
    var fields = [callerIdPrefix + callerId, md5Prefix + md5sum, typePrefix + type, latchingPrefix + latching];
    return serializeStringFields(fields);
  },
  ...
  parseTcpRosHeader: function parseTcpRosHeader(header) {
    ...
    fields.forEach(function (field) {
      var matchResult = field.match(/^(\w+)=(.+)/m);
