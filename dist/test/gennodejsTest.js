...
describe('gennodejsTests', function () {
  ...
  it('json or instance', function (done) {
    var msgData = 'chatter';
    var stdMsgString = msgUtils.getHandlerForMsgType('std_msgs/String');
    var msgInstance = new stdMsgString();
    msgInstance.data = msgData;

    var instanceBuffer = new Buffer(stdMsgString.getMessageSize(msgInstance));
    stdMsgString.serialize(msgInstance, instanceBuffer, 0);

    var jsonMsg = { data: msgData };
    var jsonBuffer = new Buffer(stdMsgString.getMessageSize(jsonMsg));
  ...
  describe('builtins', function () {
    it('string', function (done) {
      var stdMsgString = msgUtils.getHandlerForMsgType('std_msgs/String');
      var msgData = 'chatter';
      var msgSize = 4 + msgData.length;
      // manually serialize string msg
      var fullMsg = new Buffer(msgSize);
      fullMsg.writeUInt32LE(msgData.length);
      fullMsg.write(msgData, 4);

      // auto serialize
      var msg = new stdMsgString();
      msg.data = msgData;

      var fullMsg2 = new Buffer(stdMsgString.getMessageSize(msg));
    ...
    it('bool', function (done) {
      ...
      var msgBuffer = new Buffer(1);
      msgBuffer.writeInt8(true);

      var msgBuffer2 = new Buffer(1);
      var Bool = msgUtils.getHandlerForMsgType('std_msgs/Bool');
    ...
    it('int8', function (done) {
      ...
      var msgBuffer2 = new Buffer(1);
      var Int8 = msgUtils.getHandlerForMsgType('std_msgs/Int8');
    ...
    it('uint8', function (done) {
      ...
      var msgBuffer2 = new Buffer(1);
      var UInt8 = msgUtils.getHandlerForMsgType('std_msgs/UInt8');
    ...
    it('int16', function (done) {
      ...
      var msgBuffer2 = new Buffer(2);
      var Int16 = msgUtils.getHandlerForMsgType('std_msgs/Int16');
    ...
    it('uint16', function (done) {
      ...
      var msgBuffer2 = new Buffer(2);
      var UInt16 = msgUtils.getHandlerForMsgType('std_msgs/UInt16');
    ...
    it('int32', function (done) {
      ...
      var msgBuffer2 = new Buffer(4);
      var Int32 = msgUtils.getHandlerForMsgType('std_msgs/Int32');
    ...
    it('uint32', function (done) {
      ...
      var msgBuffer2 = new Buffer(4);
      var UInt32 = msgUtils.getHandlerForMsgType('std_msgs/UInt32');
    ...
    it('int64', function (done) {
      ...
      var intData = new Buffer([1, 2, 3, 0, 0, 0, 1, 6]);

      var msgBuffer = new Buffer(intData);

      var msgBuffer2 = new Buffer(8);
      var Int64 = msgUtils.getHandlerForMsgType('std_msgs/Int64');
    ...
    it('uint64', function (done) {
      ...
      var msgBuffer2 = new Buffer(8);
      var UInt64 = msgUtils.getHandlerForMsgType('std_msgs/UInt64');
    ...
    it('float32', function (done) {
      ...
      var msgBuffer2 = new Buffer(4);
      var Float32 = msgUtils.getHandlerForMsgType('std_msgs/Float32');
    ...
    it('float64', function (done) {
      var data = -3345.123576;

      var msgBuffer = new Buffer(8);
      msgBuffer.writeDoubleLE(data);

      var msgBuffer2 = new Buffer(8);
      var Float32 = msgUtils.getHandlerForMsgType('std_msgs/Float64');
      Float32.serialize({ data: data }, msgBuffer2, 0);
    ...
    it('time', function (done) {
      ...
      var msgBuffer2 = new Buffer(8);
      var Time = msgUtils.getHandlerForMsgType('std_msgs/Time');
      Time.serialize({ data: time }, msgBuffer2, 0);

      expect(msgBuffer.equals(msgBuffer2)).to.be.true;
      var deserializedTime = Time.deserialize(msgBuffer2, [0]).data;
    ...
    it('duration', function (done) {
      ...
      var msgBuffer2 = new Buffer(8);
      var Duration = msgUtils.getHandlerForMsgType('std_msgs/Duration');
      Duration.serialize({ data: duration }, msgBuffer2, 0);

      expect(msgBuffer.equals(msgBuffer2)).to.be.true;
      var deserializedDuration = Duration.deserialize(msgBuffer2, [0]).data;
  ...
  describe('complex_msgs', function () {
    it('messages and constants', function (done) {
      var BaseType = msgUtils.getHandlerForMsgType('test_msgs/BaseType');
      ...
      var newlen = 5 + BaseType.Constants.STRING_CONSTANT.length;
      expect(BaseType.getMessageSize(baseType)).to.equal(newlen);

      var msgBuffer = new Buffer(BaseType.getMessageSize(baseType));
      BaseType.serialize(baseType, msgBuffer, 0);

      var deserializedMsg = BaseType.deserialize(msgBuffer, [0]);
    ...
    it('check constant types', function (done) {
      var AllTypes = msgUtils.getHandlerForMsgType('test_msgs/AllTypes');
    ...
    it('initialization', function () {
      ...
      var stringField = 'AnInTeReStInGVaLuE';
      var numField = 12345;
      ...
      var stringField2 = 'Woahh, they changed it';
      ...
      var BaseTypeVariableLengthArray = msgUtils.getHandlerForMsgType('test_msgs/BaseTypeVariableLengthArray');
      var instance1 = new BaseTypeVariableLengthArray();
      ...
      var instance2 = new BaseTypeVariableLengthArray(instance1);
      expect(instance1.array_field).to.equal(instance2.array_field);

      var BasicService = msgUtils.getHandlerForSrvType('test_msgs/BasicService');
      var basicServiceReq = new BasicService.Request();
      ...
      var basicServiceResp = new BasicService.Response();
    ...
    it('constant length arrays', function (done) {
      var CLA = msgUtils.getHandlerForMsgType('test_msgs/ConstantLengthArray');
      ...
      cla.array_field.forEach(function (item) {
        expect(item).to.be.a('number');
        expect(item).to.equal(0);
      });

      var msgBuffer = new Buffer(CLA.getMessageSize(cla));
      CLA.serialize(cla, msgBuffer, 0);
      expect(msgBuffer.length).to.equal(claArrLen);

      var deserializedMsg = CLA.deserialize(msgBuffer, [0]);
      expect(deserializedMsg.array_field.length).to.equal(claArrLen);

      var BTCLA = msgUtils.getHandlerForMsgType('test_msgs/BaseTypeConstantLengthArray');
      var BaseType = msgUtils.getHandlerForMsgType('test_msgs/BaseType');
      ...
      btcla.array_field.forEach(function (item) {
        expect(item).to.be.an.instanceof(BaseType);
      });

      var msgBuffer2 = new Buffer(BTCLA.getMessageSize(btcla));
      BTCLA.serialize(btcla, msgBuffer2, 0);
      expect(msgBuffer2.length).to.equal(25);

      var deserializedMsg2 = BTCLA.deserialize(msgBuffer2, [0]);
      expect(deserializedMsg2.array_field.length).to.equal(btclaArrLen);
      deserializedMsg2.array_field.forEach(function (item) {
        expect(item).to.be.an.instanceof(BaseType);
      });
    ...
    it('variable length arrays', function (done) {
      var VLA = msgUtils.getHandlerForMsgType('test_msgs/VariableLengthArray');
      ...
      vla.array_field.forEach(function (item) {
        expect(item).to.be.a('number');
        expect(item).to.equal(val);
      });

      expect(VLA.getMessageSize(vla)).to.equal(arrLen + 4);
      var msgBuffer2 = new Buffer(VLA.getMessageSize(vla));
      VLA.serialize(vla, msgBuffer2, 0);

      var deserializedMsg = VLA.deserialize(msgBuffer2, [0]);
      expect(deserializedMsg.array_field.length).to.equal(arrLen);
      deserializedMsg.array_field.forEach(function (item) {
        expect(item).to.be.a('number');
        expect(item).to.equal(val);
      });

      var BTVLA = msgUtils.getHandlerForMsgType('test_msgs/BaseTypeVariableLengthArray');
      var BaseType = msgUtils.getHandlerForMsgType('test_msgs/BaseType');
      ...
      var msgBuffer3 = new Buffer(VLA.getMessageSize(btvla));
      ...
      var msgBuffer4 = new Buffer(BTVLA.getMessageSize(btvla));
      BTVLA.serialize(btvla, msgBuffer4, 0);

      var deserializedMsg2 = BTVLA.deserialize(msgBuffer4, [0]);
    ...
    it('services and constants', function (done) {
      var BasicService = msgUtils.getHandlerForSrvType('test_msgs/BasicService');
      ...
      var newlen = 8 + bsRequest.data.length + bsRequest.op.length;
      expect(BSRequest.getMessageSize(bsRequest)).to.equal(newlen);

      var msgBuffer = new Buffer(BSRequest.getMessageSize(bsRequest));
      BSRequest.serialize(bsRequest, msgBuffer, 0);

      var deserializedRequest = BSRequest.deserialize(msgBuffer, [0]);
      ...
      var BSResponse = BasicService.Response;
      ...
      var msgBuffer2 = new Buffer(BSResponse.getMessageSize(bsResponse));
      BSResponse.serialize(bsResponse, msgBuffer2, 0);
      expect(msgBuffer2.length).to.equal(8);

      var deserializedResponse = BSResponse.deserialize(msgBuffer2, [0]);
    ...
    it('service depending on this package', function (done) {
      var TestService = msgUtils.getHandlerForSrvType('test_msgs/TestService');
      var BaseType = msgUtils.getHandlerForMsgType('test_msgs/BaseType');

      var TSRequest = TestService.Request;
      var TSResponse = TestService.Response;
      expect(TSRequest).to.be.a('function');
      expect(TSResponse).to.be.a('function');

      var tsRequest = new TSRequest();
      ...
      var msgBuffer2 = new Buffer(TSResponse.getMessageSize(tsResponse));
      TSResponse.serialize(tsResponse, msgBuffer2, 0);
      expect(msgBuffer2.length).to.equal(0);

      var deserializedRequest2 = TSResponse.deserialize(msgBuffer2, [0]);
    ...
    it('message depending on another package', function (done) {
      var StdMsg = msgUtils.getHandlerForMsgType('test_msgs/StdMsg');
      var Header = msgUtils.getHandlerForMsgType('std_msgs/Header');

      var frameId = 'base';
      var time = { secs: 100, nsecs: 1000 };
      var seq = 123;
      ...
      var newlen = 24 + stdMsg.header.frame_id.length;
      expect(StdMsg.getMessageSize(stdMsg)).to.equal(newlen);
      var msgBuffer = new Buffer(StdMsg.getMessageSize(stdMsg));
      StdMsg.serialize(stdMsg, msgBuffer, 0);

      var deserializedMsg = StdMsg.deserialize(msgBuffer, [0]);
    ...
    it('service depending on another package', function (done) {
      var HeaderService = msgUtils.getHandlerForSrvType('test_msgs/HeaderService');
      var Header = msgUtils.getHandlerForMsgType('std_msgs/Header');

      var HRequest = HeaderService.Request;
      var HResponse = HeaderService.Response;
      ...
      var msgBuffer = new Buffer(HRequest.getMessageSize(hRequest));
      HRequest.serialize(hRequest, msgBuffer, 0);
      expect(msgBuffer.length).to.equal(0);

      var deserializedRequest = HRequest.deserialize(msgBuffer, [0]);
      expect(deserializedRequest).to.be.an.instanceof(HRequest);
      expect(hRequest).to.be.empty;

      var hResponse = new HResponse();
      ...
      var newlen = 16 + frameId.length;
      expect(HResponse.getMessageSize(hResponse)).to.equal(newlen);

      var msgBuffer2 = new Buffer(HResponse.getMessageSize(hResponse));
      HResponse.serialize(hResponse, msgBuffer2, 0);
      expect(msgBuffer2.length).to.equal(20);

      var deserializedRequest2 = HResponse.deserialize(msgBuffer2, [0]);
  ...
  describe('actions', function () {
    it('Action Messages Exist', function () {
  ...
  describe('Message Resolving', function () {
    // Test partially filling in a message and then being able to serialize it

    var test_msgs = msgUtils.getPackage('test_msgs');
    var Header = msgUtils.getHandlerForMsgType('std_msgs/Header');
    var BaseType = test_msgs.msg.BaseType;
    var StdMsg = test_msgs.msg.StdMsg;

    // just assuming this will be enough
    var buf = Buffer.alloc(500);

    it('Base Type', function () {
      var msg = { num_field: 1001 };
      var fullMsg = BaseType.Resolve(msg);

      expect(fullMsg.string_field).to.equal('');
      expect(fullMsg.num_field).to.equal(1001);

      expect(function () {
        return BaseType.serialize(msg, buf, 0);
      }).to.throw(Error);
      expect(function () {
        return BaseType.serialize(fullMsg, buf, 0);
      }).to.not.throw;
    ...
    it('Nested Type', function () {
      var msg = { header: { seq: 102 } };
      var fullMsg = StdMsg.Resolve(msg);

      expect(fullMsg.header instanceof Header).to.be.true;
      expect(function () {
        return StdMsg.serialize(msg, buf, 0);
      }).to.throw(Error);
      expect(function () {
        return StdMsg.serialize(fullMsg, buf, 0);
      }).to.not.throw(Error);
    ...
    it('Simple Constant Length Array', function () {
      var ConstantLengthArray = test_msgs.msg.ConstantLengthArray;

      var msg = { array_field: [1, 2] };
      var fullMsg = ConstantLengthArray.Resolve(msg);

      expect(function () {
        return ConstantLengthArray.serialize(msg, buf, 0);
      }).to.throw(Error);
      expect(function () {
        return ConstantLengthArray.serialize(fullMsg, buf, 0);
      }).to.throw(Error);
    ...
    it('Simple Variable Length Array', function () {
      var VariableLengthArray = test_msgs.msg.VariableLengthArray;

      var msg = {};
      var fullMsg = VariableLengthArray.Resolve(msg);

      expect(function () {
        return VariableLengthArray.serialize(msg, buf, 0);
      }).to.throw(Error);
      expect(function () {
        return VariableLengthArray.serialize(fullMsg, buf, 0);
      }).to.not.throw(Error);
    ...
    it('Complex Constant Length Array', function () {
      var ConstantLengthArray = test_msgs.msg.BaseTypeConstantLengthArray;

      // BaseTypeConstantLengthArray has an array_field field of length 5

      var msg = { array_field: [new BaseType(), 2] };
      var fullMsg = ConstantLengthArray.Resolve(msg);
    ...
    it('Complex Variable Length Array', function () {
      var VariableLengthArray = test_msgs.msg.BaseTypeVariableLengthArray;
